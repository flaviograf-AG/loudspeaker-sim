    ...
    <div style={{ marginTop: 8 }}>
      ...
      {way.passive_filters.map((pf, i) => {
        const updatePf = (updated: PassiveFilter) => {
          const filters = [...way.passive_filters];
          filters[i] = updated;
          onUpdate({ passive_filters: filters });
        };
        const removePf = () => onUpdate({ passive_filters: way.passive_filters.filter((_, j) => j !== i) });
      ...
      <select className="graf-form-control" style={{ width: '100%', fontSize: 12, marginTop: 4 }}
        ...
        onChange={(e) => {
          ...
          if (newFilters.length > 0) {
            onUpdate({ passive_filters: [...way.passive_filters, ...newFilters] });
          }
          e.target.value = '';
        }}>
        <option value="" disabled>+ Add component / topology...</option>
        ...
        <optgroup label="Utility"><option value="zobel">Zobel</option><option value="lpad_3db">L-Pad -3dB</option><option value="lpad_6db">L-Pad -6dB</option><option value="notch">Notch</option></optgroup>
